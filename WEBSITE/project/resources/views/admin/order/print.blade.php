            ...
            <div class="invoice__metaInfo">
                <div class="col-lg-6">
                    <div class="invoice__orderDetails">
                        ...
                        @if($order->dp == 0)
                        <span> <strong>{{ __('Delivery Method') }} :</strong>
                            @if($order->delivery == "pickup")
                            {{ __('Pick Up') }}
                            @else
                            {{ __('Deliver To Address') }}
                            @endif
                        </span><br>
                        @endif
            ...
            <div class="invoice__metaInfo" style="margin-top:0px;">
                ...
                <div class="col-lg-6">
                        <div class="invoice__orderDetails" style="margin-top:5px;">
                            <p><strong>{{ __('Delivery Details') }}</strong></p>
                           <span><strong>{{ __('Customer Name') }}</strong>: {{ $order->delivery_name == null ? $order->customer_name : $order->delivery_name}}</span><br>
                           <span><strong>{{ __('Address') }}</strong>: {{ $order->delivery_address == null ? $order->customer_address : $order->delivery_address }}</span><br>
                           <span><strong>{{ __('City') }}</strong>: {{ $order->delivery_city == null ? $order->customer_city : $order->delivery_city }}</span><br>
                           <span><strong>{{ __('Country') }}</strong>: {{ $order->delivery_country == null ? $order->customer_country : $order->delivery_country }}</span>
                        </div>
                </div>
                @endif
                            ...
                            <div class="table-responsive">
                                <table id="example2" class="table table-hover dt-responsive" cellspacing="0"
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="no-border">
                                            <td colspan="1"></td>
                                            <td><strong>{{ __('Delivery Cost') }}({{$order->currency_sign}})</strong></td>
                                            <td>{{ round($order->delivery_cost , 2) }}</td>
                                        </tr>
                                        @endif
