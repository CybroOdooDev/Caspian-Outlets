odoo.define('caspian_outlets.ProductCheck', function (require){
    "use strict";

    const PaymentScreen = require('point_of_sale.PaymentScreen');
    const Registries = require('point_of_sale.Registries');

    const ProductCheck = PaymentScreen =>
    class extends PaymentScreen {
        constructor() {
           super(...arguments);
           this.env.pos.user.allow_discount = false;
       }
       ...
       async _isOrderValid(isForceValidate) {
            ...
            for (let [key, value] of Object.entries(this.currentOrder.orderlines._byId)){
                if (value.quantity === 0){
                    ...
                    body: this.env._t(
                        'Product quantity must not be zero'
                    ),
                });
                return false;
                }
    ...
    Registries.Component.extend(PaymentScreen, ProductCheck);
    return ProductCheck;
});